...
const setup = (() => {
    ...
    function createMapFleet() {
        const container = helper.create('div',{
            className:'board-fleetContainer',
        })
    ...
    function displayWelcomeMessage() {
        const message = getElementById('message-agent');
        Component.addTypeWriteMessage(message, Message.getWelcomeMessage());
    }
    ...
    function resetArray(array) {
        for(let i = 0 ; i < array.length ; i++){
            for( let j = 0; j < array[0].length ; i++){
                array[i][j] = 'x';
            }
        }
    } 
